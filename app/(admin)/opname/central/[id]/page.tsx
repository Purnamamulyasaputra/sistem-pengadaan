        ...
        <div className="card-body flush">
          <Table>
            <thead>
              <tr>
                <th>Item Name</th>
                <th className="right">System Bal</th>
                <th className="right" style={{ width: 120 }}>Phys Qty</th>
                <th className="right" style={{ width: 80 }}>Var.</th>
                <th style={{ width: 200 }}>Reason Category</th>
                ...
                return (
                  <tr key={item.item_id}>
                    <td className="font-bold">
                      {item.item_name}
                      <div className="muted font-normal" style={{ fontSize: 12, marginTop: 4 }}>
                        MA Price: Rp {Number(item.current_average_price).toLocaleString('id-ID')}
                      </div>
                    </td>
                    <td className="right num">{Number(item.system_balance).toFixed(2)} <span className="muted">{item.smallest_unit}</span></td>
                    <td className="right">
                      <input 
                        type="number" 
                    ...
                    <td>
                      {variance !== 0 ? (
                        <select 
                          ...
                          disabled={isLocked}
                          style={{ height: 32, padding: '4px 8px', fontSize: 13, borderColor: !detail?.reason_category ? '#fca5a5' : 'var(--border)' }}
                        >
                          <option value="">-- Select Reason --</option>
                          {REASON_CATEGORIES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                        </select>
                      ) : (
                        <span className="muted italic" style={{ fontSize: 13 }}>No variance</span>
                      )}
                    </td>
                    <td>
                      {variance !== 0 && detail?.reason_category ? (
                        <input 
                          ...
                          style={{ height: 32, padding: '4px 8px', fontSize: 13, width: '100%', borderColor: (detail?.reason_category === 'LAINNYA' && !detail?.reason_notes?.trim()) ? '#fca5a5' : 'var(--border)' }}
                        />
                      ) : null}
                    </td>
                  </tr>
